...
// keep the rest of file as you have it
router.post("/import", upload.single("file"), async (req, res) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      await upsertLead({ name, email, phone, tags, note }); // must throw code "23505" on existing
      inserted++;
    } catch (e: any) {
  ...
  return res.json({
    ok: true,
    inserted,
    duplicates: dbDuplicates,   // already uploaded in DB
    invalids,
    skipped,
    errors,
    ...
    meta: {
      ...
      fileDuplicates,           // in-file duplicates
    },
    confidence,
  });
});
