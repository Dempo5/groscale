...
/* ======================================================================
   Conversations / Messages (single, normalized API)
   ====================================================================== */
...
export type MsgStatus = "QUEUED" | "SENT" | "DELIVERED" | "FAILED" | "RECEIVED";
...
// normalize arrays regardless of server envelope
function pickArray<T = unknown>(res: any, key: string): T[] {
...
/** List all threads (normalized to Thread[]) */
export async function listThreads(): Promise<Thread[]> {
  const res = await http<any>("/api/messages/threads");
  return pickArray<Thread>(res, "threads");
}

/** Get a thread + its messages */
export async function getThread(
  threadId: string
): Promise<{ thread: Thread; messages: MessageDTO[] }> {
  const res = await http<any>(`/api/messages/threads/${threadId}`);
  const payload = res?.data ?? res;
  return {
    thread: payload?.thread as Thread,
    messages: pickArray<MessageDTO>(payload, "messages"),
  };
}

/** Get only the messages for a thread (if you prefer this split) */
export async function getThreadMessages(threadId: string): Promise<MessageDTO[]> {
  const res = await http<any>(`/api/messages/${threadId}`);
  return pickArray<MessageDTO>(res, "messages");
}

/** Send a message.
 * - Provide `threadId` to reply in an existing thread
 * - Or provide `to` (E.164) to start a new thread
 * - Optionally include `leadId` if your server wants to associate a lead
 */
export async function sendMessage(input: {
  ...
  body: string;
  leadId?: string;
}): Promise<{ ok: boolean; message?: MessageDTO; threadId?: string; error?: string }> {
  return http("/api/messages/send", {
    method: "POST",
    body: JSON.stringify(input),
  });
}

/** Convenience: start a thread and optionally send the first message */
export async function startThread(input: {
  phone: string;  // E.164
  name?: string;
  leadId?: string;
  body?: string;  // optional first message body
}): Promise<{ ok: boolean; thread: Thread; message?: MessageDTO; error?: string }> {
  return http("/api/messages/start", {
    method: "POST",
    body: JSON.stringify(input),
  });
}
